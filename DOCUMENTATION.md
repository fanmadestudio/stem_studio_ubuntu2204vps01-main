...
| Secure access | Use JWT-based authentication and role-based API permissions |
...
|---|---|
| Authentication | Login, token refresh, user registration API, profile retrieval, profile update |
...
|---|---|---|
| Admin | User CRUD API, client CRUD, resource CRUD, booking CRUD, invoice CRUD/actions, payment CRUD including delete, notification CRUD, Django admin access when staff/superuser flags are set | Must authenticate with JWT or Django admin session |
...
| Process | Frontend posts credentials to `/api/v1/auth/token/`; backend validates credentials and returns JWT access/refresh tokens |
| Output | Access token, refresh token, and user metadata |
| Validation | Password must match stored hash; backend authenticates through SimpleJWT |
| Technical notes | Frontend stores `access`, `refresh`, display name, and auth expiry values in `localStorage` |
...
3. Frontend stores JWT credentials and redirects to dashboard.
...
flowchart TD
    ...
    C -- "Yes" --> E["Store JWT in localStorage"]
    E --> F["Dashboard"]
    F --> G["Manage Clients"]
...
|---|---|---|
| Frontend | React 19, TypeScript, Vite | User interface, client-side routing, API calls, token storage |
| Backend API | Django 4.2, Django REST Framework | Business logic, validation, authentication, authorization, persistence |
| Authentication | SimpleJWT | Access/refresh token generation and validation |
...
flowchart TD
    ...
    API --> Auth["SimpleJWT Authentication"]
...
| JWT authentication | `djangorestframework-simplejwt` | `5.5.1` | Token authentication |
...
| Path | Function |
|---|---|
| `backend/config/settings.py` | Django settings, installed apps, database, JWT, CORS, pagination |
...
| `frontend/app/lib/api.ts` | Fetch wrapper, JWT header injection, pagination helper |
...
| Authentication storage | JWT access and refresh tokens are stored in `localStorage` |
...
| POST | `/api/v1/auth/token/` | Login and obtain JWT tokens | Public |
| POST | `/api/v1/auth/token/refresh/` | Refresh access token | Public with refresh token |
...
STEM Studio uses JWT authentication through Django REST Framework SimpleJWT.

### Login

1. User submits email and password.
2. Frontend sends a POST request to `/api/v1/auth/token/`.
3. Backend returns access token, refresh token, and user metadata.
4. Frontend stores token data in `localStorage`.
5. Subsequent API requests include `Authorization: Bearer <access-token>`.
...
The frontend wraps application pages with `AuthGate`. If no access token is found or the stored expiry is invalid, the user is redirected to the login page.
...
The frontend logout flow clears token and user display values from `localStorage`. The repository does not currently define a backend logout endpoint; token invalidation depends on token expiry/rotation behavior.
...
| Authentication errors | DRF/SimpleJWT returns `401 Unauthorized` for missing/invalid tokens |
...
| JWT authentication | SimpleJWT access/refresh tokens | Consider longer refresh lifetime only if supported by secure logout/blacklist policy |
| Token storage | Frontend stores JWTs in `localStorage` | Be strict about XSS prevention because localStorage is script-readable |
| CORS | Configurable `DJANGO_CORS_ALLOWED_ORIGINS`; optional allow-all in debug edge case | Restrict origins in production |
| CSRF | Django CSRF middleware enabled; JWT APIs primarily use Authorization header | Keep trusted origins aligned with deployed frontend |
...
### 24.4 Frontend Production Setup
...
### 24.5 Domain, Reverse Proxy, and HTTPS

The setup script installs Nginx but does not include a complete Nginx virtual host file. A production deployment should add a reverse proxy similar to:
...
| 401 Unauthorized | Missing/expired JWT | Log in again or refresh token |
...
| LocalStorage token storage | Simple implementation but more exposed to XSS than HttpOnly cookies |
